...
const EditForm: React.FC<PropType> = (props) => {
    ...
    const [currentFigure, setCurrentFigure] = useState<Figure | undefined>(
        undefined
    )
    const [currentFigureImg, setCurrentFigureImg] = useState<
        string | undefined
    >('')
    ...
    // FETCH DATA
    const BA20132014Figures: Figure[] = useBA20132014Store(
        (state) => state.figures
    )
    const BA20142015Figures: Figure[] = useBA20142015Store(
        (state) => state.figures
    )
    const BA20152017Figures: Figure[] = useBA20152017Store(
        (state) => state.figures
    )
    const EX20142015Figures: Figure[] = useEX20142015Store(
        (state) => state.figures
    )
    const MU20152017Figures: Figure[] = useMU20152017Store(
        (state) => state.figures
    )
    ...
    useEffect(() => {
        switch (collection) {
            case 'ba20132014':
                const currentBA20132014 = BA20132014Figures.find(
                    (item) => item.id === id
                )
                ...
                break
            case 'ba20142015':
                const currentBA20142015 = BA20142015Figures.find(
                    (item) => item.id === id
                )
                ...
                break
            case 'ba20152017':
                const currentBA20152017 = BA20152017Figures.find(
                    (item) => item.id === id
                )
                ...
                break
            case 'ex20142015':
                const currentEX20142015 = EX20142015Figures.find(
                    (item) => item.id === id
                )
                ...
                break
            case 'mu20152017':
                const currentMU20152017 = MU20152017Figures.find(
                    (item) => item.id === id
                )
    ...
    // FORMS
    const updateFigureBA20132014 = useBA20132014Store(
        (state) => state.updateFigure
    )
    const updateFigureBA20142015 = useBA20142015Store(
        (state) => state.updateFigure
    )
    const updateFigureBA20152017 = useBA20152017Store(
        (state) => state.updateFigure
    )
    const updateFigureEX20142015 = useEX20142015Store(
        (state) => state.updateFigure
    )
    const updateFigureMU20152017 = useMU20152017Store(
        (state) => state.updateFigure
    )
        ...
        switch (collection) {
            case 'ba20132014':
                updateFigureBA20132014(
                    ...
                    selectedFile ? selectedFile?.file : currentFigureImg
                ).then((success) => {
                    if (success) {
    ...
    const deleteFigureBA20142015 = useBA20142015Store(
        (state) => state.deleteFigure
    )
    const deleteFigureBA20152017 = useBA20152017Store(
        (state) => state.deleteFigure
    )
    const deleteFigureEX20142015 = useEX20142015Store(
        (state) => state.deleteFigure
    )
    const deleteFigureMU20152017 = useMU20152017Store(
        (state) => state.deleteFigure
    )
    ...
    const handleDelete = () => {
        ...
        switch (collection) {
            case 'ba20132014':
                deleteFigureBA20132014(id, currentFigure?.photoRef || '').then(
                    (success) => {
                        if (success) {
                            router.push('dashboard')
                        }
                ...
                break
            case 'ba20142015':
                deleteFigureBA20142015(id, currentFigure?.photoRef || '').then(
                    (success) => {
                        if (success) {
                            router.push('dashboard')
                        }
                ...
                break
            case 'ba20152017':
                deleteFigureBA20152017(id, currentFigure?.photoRef || '').then(
                    (success) => {
                        if (success) {
                            router.push('dashboard')
                        }
                ...
                break
            case 'ex20142015':
                deleteFigureEX20142015(id, currentFigure?.photoRef || '').then(
                    (success) => {
                        if (success) {
                            router.push('dashboard')
                        }
                ...
                break
            case 'mu20152017':
                deleteFigureMU20152017(id, currentFigure?.photoRef || '').then(
                    (success) => {
                        if (success) {
                            router.push('dashboard')
                        }
                        ...
                        <Form className="w-full flex items-center justify-between">
                            ...
                            <div className="w-[30%] flex flex-col items-center justify-between">
                                ...
                                <div className="flex justify-between w-[80%]">
                                    ...
                                    <button
                                        className="px-[45px] py-[15px] inline-block rounded-full bg-[#F71A09]"
                                        onClick={handleDelete}
                                    >
                                        <div className="flex items-center">
                                            <div className="text-[16px] text-white text-center font-bold">
                                                Delete
                                            </div>
                                        </div>
                                    </button>
