...
const BA20142015Store = (set: any) => ({
    ...
    addFigure: async(newFigure: any, currentFile: any) => {
        ...
        try {
            ...
            if (currentFile) {
                fileRefName = `BA20142015/${uuidv4()}-${currentFile.name}`;
            ...
            await addDoc(collection(db, 'BA20142015'), {
                ...newFigure,
    ...
    updateFigure: async(documentId: any, updatedFigure: any, newFile: any) => {
        ...
        try{
            ...
            if (currentFigure?.photoUrl !== newFile?.source) {
                ...
                if (newFile) {
                    {
                        fileRefName = `BA20142015/${uuidv4()}-${newFile.file.name}`;
                        const imageRef = ref(storage, fileRefName);
...
export const useBA20142015Store = create(BA20142015Store);