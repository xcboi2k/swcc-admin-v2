...
const BA20152017Store = (set: any) => ({
    ...
    addFigure: async(newFigure: any, currentFile: any) => {
        ...
        try {
            ...
            if (currentFile) {
                fileRefName = `BA20152017/${uuidv4()}-${currentFile.name}`;
            ...
            await addDoc(collection(db, 'BA20152017'), {
                ...newFigure,
    ...
    updateFigure: async(documentId: any, updatedFigure: any, newFile: any) => {
        ...
        try{
            const docRef = doc(db, 'BA20152017', documentId);
            ...
            if (currentFigure?.photoUrl !== newFile?.source) {
                ...
                if (newFile) {
                    {
                        fileRefName = `BA20152017/${uuidv4()}-${newFile.file.name}`;
                        const imageRef = ref(storage, fileRefName);
...
export const useBA20152017Store = create(BA20152017Store);